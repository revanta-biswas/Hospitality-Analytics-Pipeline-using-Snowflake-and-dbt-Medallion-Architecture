...
| **SH-LOOP-10** | CI Attestation (post-PR provisioning agreement) | Section D Step 8 | CI's `gates` block agrees with this story's own local gate results on every root the diff touched — no gate the diff touched is absent, `N/A`, or provisioning-errored in CI when it passed locally |
