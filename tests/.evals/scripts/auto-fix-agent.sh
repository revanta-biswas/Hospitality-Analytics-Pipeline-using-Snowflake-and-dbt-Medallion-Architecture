...
#    Resolved against Claude Code CLI v2.1.270 (the same pin used for "Install Claude Code CLI", Section
#    6.0.2): -p/--print for non-interactive output (no TTY exists on this runner); the ephemeral runner
#    justifies --dangerously-skip-permissions (Section 6.0's own "broader permissions more defensible on
#    an ephemeral runner" guidance) so a tool-call approval prompt never hangs the job; --allowedTools
#    scopes the agent to file edits and the repo's own git/test commands, never broader.
# >>> CLAUDE_REPAIR_INVOCATION START <<<
printf '%s' "$BRIEF" | claude -p --dangerously-skip-permissions \
  --allowedTools "Read,Edit,Write,Bash(git *),Bash(dbt *),Bash(pytest*),Bash(pip*)" \
