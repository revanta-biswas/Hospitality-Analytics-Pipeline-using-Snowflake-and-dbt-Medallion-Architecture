...
${diff}"
  ...
  #    Resolved against Claude Code CLI v2.1.270 (Section 6.0.2 pin): -p/--print for non-interactive
  #    output; --model "$MODEL" pins the judge to config.json's judge.model rather than the session
  #    default; --dangerously-skip-permissions because this is a read-only scoring call on an ephemeral
  #    runner with no human available to approve a tool call. Deliberately NOT --output-format json — the
  #    extraction below expects the model's own prose-wrapped JSON object via sed/jq, and the CLI's own
  #    --output-format json would wrap that same text inside an additional {"result": "..."} envelope,
  #    breaking this parser rather than simplifying it.
  local rc claude_err_file
  claude_err_file="$(mktemp)"
  # >>> CLAUDE_JUDGE_INVOCATION START <<<
  resp="$(printf '%s' "$prompt" | claude -p --model "$MODEL" --dangerously-skip-permissions 2>"$claude_err_file")"; rc=$?
