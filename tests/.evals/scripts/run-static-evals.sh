...
# >>> STACK-RESOLVED D-GATES START <<<
  # Root "." (stack: python, markerFile: pyproject.toml, tests/.evals/config.json ci.roots[0]).
  # Commands resolved per common/ci-pipeline-generation.md Section 3's python column. This repo has
  # zero .py files today (dbt/SQL project) — these still run for real, against sourcePaths, and report
  # a genuine tool-verified clean result rather than a fabricated N/A; the moment a story adds a .py
  # file under src/dbt_code or src/snowflake, these gates measure it for real with no re-generation.
  # D1_lint — ruff, JSON output normalized to rule\tfile\tmessage. The `paths=""; for d in ...` guard
  # pre-filters to only the sourcePaths that actually exist at THIS checkout — a baseline capture
  # against an older ref where src/dbt_code or src/snowflake did not exist yet (this repo's own history:
  # they were only moved under src/ in a later commit) must record a true "nothing existed to scan"
  # result, never a tool crash on a missing path (Section 4.0d.1 rule 6's "a root can legitimately not
  # exist yet at an older base ref").
  delta_diff D1_lint 'paths=""; for d in src/dbt_code src/snowflake; do [ -d "$d" ] && paths="$paths $d"; done; if [ -n "$paths" ]; then ruff check --output-format json $paths 2>/dev/null | jq -r ".[] | [.code, .filename, .message] | @tsv"; fi' "." "pyproject.toml"
  # D2_types — mypy, plain text "file:line: error: message [code]" normalized to rule\tfile\tmessage.
  # Same existing-paths guard as D1_lint above, PLUS a ".py file actually present" guard: mypy exits 2
  # ("no files found") rather than 0 when a resolved path exists but has no .py file under it (this
  # repo's own state today — pure dbt/SQL, zero .py files), and this script runs under `set -o pipefail`
  # (line 18), so that 2 would otherwise outrank the downstream grep/sed exit and get misread as a tool
  # crash (ERROR) instead of the true "nothing to type-check yet" result. `find -name '*.py'` decides
  # whether to invoke mypy at all, so a genuine mypy crash (bad config, unsupported flag) still surfaces.
  delta_diff D2_types 'paths=""; for d in src/dbt_code src/snowflake; do [ -d "$d" ] && paths="$paths $d"; done; if [ -n "$paths" ] && find $paths -name "*.py" -print -quit 2>/dev/null | grep -q .; then mypy --ignore-missing-imports $paths 2>/dev/null | grep ": error:" | sed -E "s/^([^:]+):[0-9]+: error: (.*) \[([a-z0-9_-]+)\]$/\3\t\1\t\2/"; fi' "." "pyproject.toml"
  # D4_deps — pip-audit, JSON output normalized to rule\tfile\tmessage (file is always the resolved
  # dependency graph, not a source path — this gate scans installed packages, not sourcePaths).
  delta_diff D4_deps 'pip-audit --format json 2>/dev/null | jq -r ".dependencies[]? | select(.vulns and (.vulns | length) > 0) | .vulns[] | [.id, \"pyproject.toml\", (.description // \"vulnerable dependency\")] | @tsv"' "." "pyproject.toml"
  # D5_licenses — pip-licenses, filtered against AIRE_DISALLOWED_LICENSES (Section 3.0c) — never the
  # literal threshold copied inline; both are read from tests/.evals/config.json at run time via the env
  # var this same script already exports.
  delta_diff D5_licenses 'pip-licenses --format json 2>/dev/null | jq -r --arg disallowed "$AIRE_DISALLOWED_LICENSES" "(\$disallowed | split(\",\")) as \$bad | .[] | select(.License as \$l | \$bad | index(\$l) != null) | [.License, .Name, (\"disallowed license \" + .License + \" for \" + .Name)] | @tsv"' "." "pyproject.toml"
  # D6_complexity — radon cc, filtered against AIRE_MAX_CYCLOMATIC_COMPLEXITY (Section 3.0c).
  delta_diff D6_complexity 'radon cc -j src/dbt_code src/snowflake 2>/dev/null | jq -r --argjson max "$AIRE_MAX_CYCLOMATIC_COMPLEXITY" "to_entries[] | .key as \$file | .value[] | select(.complexity > \$max) | [(\"complexity-\" + (.complexity|tostring)), \$file, (.name + \" complexity \" + (.complexity|tostring) + \" exceeds max \" + (\$max|tostring))] | @tsv"' "." "pyproject.toml"
